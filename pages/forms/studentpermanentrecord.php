...
//   dagdag eto 
$l= mysqli_query($db, "SELECT * FROM tbl_students 
LEFT JOIN tbl_schoolyears ON tbl_schoolyears.stud_id = tbl_students.stud_id WHERE tbl_schoolyears.stud_id = '$_GET[stud]' AND sem_id = '$_GET[sem]' AND ay_id = '$_GET[sy]'");
...
$sum = mysqli_query($db,"SELECT SUM(unit_total) as UN FROM tbl_enrolled_subjects LEFT JOIN tbl_subjects_new ON tbl_subjects_new.subj_id = tbl_enrolled_subjects.subj_id where tbl_enrolled_subjects.stud_id = '$_GET[stud]' AND tbl_enrolled_subjects.acad_year = '$_GET[sy]' AND tbl_enrolled_subjects.semester = '$_GET[sem]' AND tbl_subjects_new.course_id = '$rows[course_id]'");
                          $row = mysqli_fetch_array ($sum);

$credited = mysqli_query($db,
"SELECT SUM(unit_total) as UN 
FROM tbl_enrolled_subjects 
LEFT JOIN tbl_subjects_new ON tbl_subjects_new.subj_id = tbl_enrolled_subjects.subj_id where remarks = 'Passed' 
AND tbl_enrolled_subjects.stud_id = '$_GET[stud]' 
AND tbl_enrolled_subjects.acad_year = '$_GET[sy]' 
AND tbl_enrolled_subjects.semester = '$_GET[sem]'
AND tbl_subjects_new.course_id = '$rows[course_id]'
...
$sqls = mysqli_query($db,"SELECT *,tbl_subjects_new.subj_code,tbl_subjects_new.subj_desc FROM tbl_enrolled_subjects
  ...
  where tbl_enrolled_subjects.acad_year = '$_GET[sy]' AND tbl_enrolled_subjects.semester = '$_GET[sem]' AND stud_id = '$_GET[stud]' AND tbl_subjects_new.course_id = '$rows[course_id]'")or die($db);
...
$pdf ->SetFont('Arial','B',9);
$pdf ->Cell(79,5,'Rowena Bondoc',0,0,'C');
$pdf ->SetFont('Arial','',9);
...
$pdf ->SetFont('Arial','',9);
$pdf ->Cell(20,5,'',0,0);
$pdf ->Cell(79,5,'Chief Evaluator',0,0,'C');
$pdf ->Cell(18,5,'',0,0);
